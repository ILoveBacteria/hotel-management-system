	...
	async function handlePayment(billId: number) {
		try {
			const response = await fetch(
				`${PUBLIC_BASE_URL}/payments/bills/${billId}/pay/`,
				{
					method: 'POST',
					credentials: 'include',
					headers: {
			...
			// Handle successful payment - may redirect to payment gateway
			if (response.status === 302) {
				// Handle redirect if payment gateway is used
		...
		<div class="grid grid-cols-1 md:grid-cols-3 gap-6">
			...
			<div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
				<div class="flex items-center justify-between">
					...
					<div
						class="p-3 bg-yellow-100 dark:bg-yellow-900/20 rounded-full"
					>
